...
**OTLP ingestion (v3.3 metrics, v3.4 logs + traces):** `POST /v1/metrics`, `POST /v1/logs`, `POST /v1/traces` — mounted at the OpenTelemetry spec's own paths, deliberately NOT under `/api/v1`, because every exporter appends the signal path to its endpoint. Protobuf only (`application/x-protobuf`), gzip supported with a decompression-bomb ceiling enforced *during* inflation, OTLP partial success, `google.rpc.Status` error bodies. Metrics store gauge and sum (delta/cumulative) points. Logs store severity (number, text and an indexed band), body, attributes and the `trace_id`/`span_id` that make a log line reachable from a span. Spans are stored **flat** with a parent pointer — children routinely arrive before parents, so the waterfall is assembled at read time; `(organization_id, trace_id, span_id)` is unique, making a collector's retry idempotent. Authenticated by an organisation-scoped `IngestCredential` (`sxi_live_`) with **one scope per signal** (`metrics:write`, `logs:write`, `traces:write`) so a trace collector's key cannot write metrics. Sensitive attribute keys (`authorization`, `api_key`, `set-cookie`, …) are redacted before storage and the redacted key names recorded — matching is on keys only, never bodies, because scanning application text for secret-shaped strings destroys legitimate logs and still misses real leaks (`INGEST_SENSITIVE_ATTRIBUTE_MODE=redact|reject`). Retention: logs 14 days, spans 7, pruned by the worker. Not implemented, and advertised as such in `/health`: gRPC, OTLP/JSON, histograms, exponential histograms, summaries, span links. See `docs/adr/0011-otlp-interoperability-boundary.md`.

**Log & trace exploration (v3.4):** `GET /api/v1/signals/logs` (+ `/logs/facets`), `GET /api/v1/signals/traces`, `GET /api/v1/signals/traces/{trace_id}` and `GET /api/v1/signals/traces/{trace_id}/logs`. Keyset pagination on `(time, id)` rather than OFFSET, so page one thousand costs what page one costs; a corrupt cursor is refused rather than silently restarting. Body search is a bounded, escaped substring match paired with a time range — never regex. Trace filters select *traces* and then summarise the whole trace, so filtering by `status=error` still reports the root operation and full span count. Span depth and waterfall offsets are derived at read time with a cycle guard. A trace id belonging to another tenant reports `found: false` rather than 404, so the endpoint is not an existence oracle.
